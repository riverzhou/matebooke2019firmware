...
WIN_MNT="/mnt/c"									# Windows mount path
...
# Copy DSP files
for DSP_FILE in `find /mnt/c/Windows/System32/DriverStore/FileRepository/ -name qcadsp850.mbn`; do
	DSP_PATH=`dirname "${DSP_FILE}"`
	cp -a "${DSP_PATH}" "${PATH_WIN_DRV}" 
	if [ $? -ne 0 ]; then
		COPY_ERR=$((COPY_ERR+1))
	fi
done
echo "qcadsp850.mbn" ${COPY_ERR}
# Copy Shader file
for SHADE_FILE in `find /mnt/c/Windows/System32/DriverStore/FileRepository/ -name qcdxkmsuc850.mbn`; do
	SHADE_PATH=`dirname "${SHADE_FILE}"`
	cp -a "${SHADE_PATH}" "${PATH_WIN_DRV}"
	if [ $? -ne 0 ]; then
		COPY_ERR=$((COPY_ERR+1))
	fi
done
echo "qcdxkmsuc850.mbn" ${COPY_ERR}
#venus decoder
for VEN_FILE in `find /mnt/c/Windows/System32/DriverStore/FileRepository/ -name qcvss850.mbn`; do
	VEN_PATH=`dirname "${VEN_FILE}"`
	cp -a "${VEN_PATH}" "${PATH_WIN_DRV}"
	if [ $? -ne 0 ]; then
		COPY_ERR=$((COPY_ERR+1))
	fi
done
echo "qcvss850.mbn" ${COPY_ERR}
# ipa firmware
for IPA_FILE in `find /mnt/c/Windows/System32/DriverStore/FileRepository/ -name ipa_fws.elf`; do
	IPA_PATH=`dirname "${IPA_FILE}"`
	cp -a "${IPA_PATH}" "${PATH_WIN_DRV}"
	if [ $? -ne 0 ]; then
		COPY_ERR=$((COPY_ERR+1))
	fi
done
echo "ipa_fws.elf" ${COPY_ERR}
# Copy board files
for BRD_FILE in `find /mnt/c/Windows/System32/DriverStore/FileRepository/ -name bdwlan.bin`; do
...
#			file_ext="0"
	...
	git clone --depth=1 https://github.com/qca/qca-swiss-army-knife.git 
	done_failedexit $?

	echo -n "Creating merged board file: "
	python3 qca-swiss-army-knife/tools/scripts/ath10k/ath10k-bdencoder -c "${JSON}" -o "${PATH_BRD_MFILE}" 
	done_failedexit $?
###################################################################################################################################################
	cd "${CWD}"
...
###################################################################################################################################################
	...
	PATH_FW_C630="${CWD}/sdm850/HUAWEI/AL09"
	...
	mkdir -p "${PATH_FW_C630}" 
	#done_failedexit $?
	echo -n "Copying linux DSP files: "
	cp -a "${DSP_PATH}"/*.mbn "${PATH_FW_C630}" 
	#done_failedexit $?
	cp -a "${SHADE_PATH}"/*.mbn "${PATH_FW_C630}" 
	#done_failedexit $?
	cp -a "${VEN_PATH}"/*.mbn "${PATH_FW_C630}" 
	#done_failedexit $?
	cp -a "${IPA_PATH}"/*.elf "${PATH_FW_C630}" 
	#done_failedexit $?
	cd "${PATH_FW_C630}"
	echo -n "Creating symlink qcdsp2850.mbn -> modem.mdt: "
	ln -s qcdsp2850.mbn modem.mdt 
	#done_failedexit $?
	cd "${CWD}"
...
# split ipa and venus
	...
	python3 pil-splitter.py sdm850/HUAWEI/AL09/ipa_fws.elf ipa_fws
	echo "splitting qcvss850.mbn"
    	python3 pil-splitter.py sdm850/HUAWEI/AL09/qcvss850.mbn venus 
...
###################################################################################################################################################
	...
	PATH_LIBFW_ATH10K="/home/river/firmware/ath10k"
	echo -n "Copying new Atheros ath10k firmware: "
	sudo mkdir -p /home/river/firmware/
	sudo cp -r "${PATH_FW_ATH10K}" "${PATH_LIBFW_ATH10K}" 
	sudo chown -R root:root "${PATH_LIBFW_ATH10K}"				# Reset ownership

	PATH_LIBFW_QCOM="/home/river/firmware/qcom"
	echo -n "Copying new Qualcomm DSP firmware: "
	sudo mkdir -p /home/river/firmware/qcom/venus-5.2/
	sudo mkdir -p /home/river/firmware/ath10k/WCN3990/hw1.0/
	sudo mkdir -p /home/river/firmware/qcom/sdm850/HUAWEI/AL09/
	sudo cp ./ipa_fws* /home/river/firmware/
	sudo cp venus* /home/river/firmware/qcom/venus-5.2/
	sudo cp -r "sdm850" "${PATH_LIBFW_QCOM}" 
	sudo cp creating-board-2.bin/board-2.bin /home/river/firmware/ath10k/WCN3990/hw1.0/
    	sudo cp ${PATH_FW_C630}/modem.mdt /home/river/firmware/qcom/sdm850/HUAWEI/AL09/modem.mdt
	sudo cp linux-firmware/qcom/sdm845/*.jsn /home/river/firmware/qcom/sdm850/HUAWEI/AL09/
	sudo cp linux-firmware/ath10k/WCN3990/hw1.0/firmware-5.bin /home/river/firmware/ath10k/WCN3990/hw1.0/firmware-5.bin
	sudo cp linux-firmware/ath10k/WCN3990/hw1.0/wlanmdsp.mbn /home/river/firmware/ath10k/WCN3990/hw1.0/wlanmdsp.mbn
	sudo cp linux-firmware/ath10k/WCN3990/hw1.0/wlanmdsp.mbn /home/river/firmware/qcom/sdm850/HUAWEI/AL09/wlanmdsp.mbn
	sudo cp linux-firmware/ath10k/WCN3990/hw1.0/wlanmdsp.mbn /home/river/firmware/qcom/wlanmdsp.mbn
	sudo cp linux-firmware/qcom/a630_gmu.bin /home/river/firmware/qcom/
	sudo cp linux-firmware/qcom/a630_sqe.fw  /home/river/firmware/qcom/
